...
-- Add column to distinguish admin-granted submissions
ALTER TABLE public.submissions ADD COLUMN is_admin_granted boolean NOT NULL DEFAULT false;

-- Update leaderboard function to exclude admin-granted from last_solve
...
AS $function$
  SELECT 
    ...
    (SELECT MAX(s5.submitted_at) 
     ...
     WHERE s5.user_id = p.id AND s5.is_correct = true AND s5.is_admin_granted = false
    ) AS last_solve,
    p.player1_name,
    p.player2_name,
    p.player3_name
